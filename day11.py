...
def load(file):
  with open(file) as f:
    return [zeile.replace(':', '').split() for zeile in f]


@functools.cache
def count(node, target):
  return node == target or sum(count(next, target) for next in devices[node])
...
p = load('day11.txt')
devices = {node: outputs for node, *outputs in p} | {'out': []}
